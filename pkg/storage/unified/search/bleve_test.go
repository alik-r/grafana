...
func TestBleveInMemoryIndexExpiration(t *testing.T) {
	...
	builtIndex, err := backend.BuildIndex(context.Background(), ns, 1 /* below FileThreshold */, 100, nil, "test", indexTestDocs(ns, 1), nil)
	require.NoError(t, err)
...
func TestBleveFileIndexExpiration(t *testing.T) {
	...
	builtIndex, err := backend.BuildIndex(context.Background(), ns, 100, 100, nil, "test", indexTestDocs(ns, 1), nil)
	require.NoError(t, err)
...
func TestFileIndexIsReusedOnSameSizeAndRV(t *testing.T) {
	...
	_, err := backend1.BuildIndex(context.Background(), ns, 10 /* file based */, 100, nil, "test", indexTestDocs(ns, 10), nil)
	require.NoError(t, err)
	...
	idx, err := backend2.BuildIndex(context.Background(), ns, 10 /* file based */, 100, nil, "test", indexTestDocs(ns, 1000), nil)
	require.NoError(t, err)
...
func TestFileIndexIsNotReusedOnDifferentSize(t *testing.T) {
	...
	_, err := backend1.BuildIndex(context.Background(), ns, 10, 100, nil, "test", indexTestDocs(ns, 10), nil)
	require.NoError(t, err)
	backend1.closeAllIndexes()
	...
	idx, err := backend2.BuildIndex(context.Background(), ns, 100, 100, nil, "test", indexTestDocs(ns, 100), nil)
	require.NoError(t, err)
...
func TestFileIndexIsNotReusedOnDifferentRV(t *testing.T) {
	...
	backend2, _ := setupBleveBackend(t, 5, time.Nanosecond, tmpDir)
	idx, err := backend2.BuildIndex(context.Background(), ns, 10 /* file based */, 999999, nil, "test", indexTestDocs(ns, 100), nil)
	require.NoError(t, err)
...
func TestRebuildingIndexClosesPreviousCachedIndex(t *testing.T) {
	...
	for name, testCase := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			firstIndex, err := backend.BuildIndex(context.Background(), ns, int64(firstSize), 100, nil, "test", indexTestDocs(ns, firstSize), nil)
			require.NoError(t, err)
			...
			secondIndex, err := backend.BuildIndex(context.Background(), ns, int64(secondSize), 100, nil, "test", indexTestDocs(ns, secondSize), nil)
			require.NoError(t, err)
...
func testBleveIndexWithFailures(t *testing.T, fileBased bool) {
	...
	_, err = backend.BuildIndex(context.Background(), ns, size, 100, nil, "test", indexTestDocs(ns, int(size)), nil)
	require.NoError(t, err)
}
